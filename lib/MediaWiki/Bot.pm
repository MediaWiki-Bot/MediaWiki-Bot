...
is returned if no result was found, as would be the case if the page is deleted or
...
Returns the wikitext of the specified C<$page_title>. It's similar to L</get_page>.
See L</get_page> for differences.
...
keys, where C<page> (or C<pageid> respectively) and I<text> are the only required
ones:
...
C<page> - the page title to edit (required unless C<pageid> is given)

=item *

C<pageid> - instead of C<page> you may give the pageid. If both is given, then
C<page> is preferred.

=item *

C<text> - the page text to write (required)

=item *

C<summary> - an edit summary (default: some useless message, so better define this)

=item *

C<minor> - whether to mark the edit as minor (boolean, default = 1)

=item *

C<bot> - whether to mark the edit as a bot edit (boolean, default = 1)

=item *

C<assertion> - usually 'bot', but see
L<http://mediawiki.org/wiki/Extension:AssertEdit>.

=item *

C<section> - edit a single section (identified by number) instead of the whole
page. If set to 'new', a new section will be appended to the page. That new
section will be titled with the name given by the param C<sectiontitle>.

=item *

C<sectiontitle> - a section title of a new section, see C<section> param.

=item *

C<edittoken> - used to detect edit conflicts. It is an optional parameter normally
...
An optional forth parameter C<$edittoken> can be used to detect edit conflicts.
